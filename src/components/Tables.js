...
import {baseUrl, getNextPaginationUrl, getPagination, getPaginationUrl} from "../util/Util";
...
export const CommonTable = (props) => {
    ...
    if (!ready) {
        return <Preloader loaded={ready ? false : true}/>
    }
                ...
                <Table hover className="user-table align-items-center">
                    ...
                    <tr>
                        {fields.length > 0 && fields.map((field, i) => (
                                <th key={`header-${field}-${i}`} className="border-bottom">{field}</th>
                            )
                        )}
                    </tr>
                    </thead>
                    <tbody>
                    {data.length > 0 && data.map((t, i) => <TableRow key={`transaction-${i}`} row={t}/>)}
                    </tbody>
                </Table>
                <Card.Footer className="px-3 border-0 d-lg-flex align-items-center justify-content-between">
                    <Nav>
                        <Pagination className="mb-2 mb-lg-0">
                            ...
                            {getPagination(currentPage, totalCount).map(page =>
                                <Pagination.Item key={`pagination-${page}`} active={page === parseInt(currentPage)} onClick={() => handlePageChange(page)}>
                                    {page}
                                </Pagination.Item>
                            )}
